package com.tan.transaction.service;
...
public class DemoService {
    ...
    public void outTransExecute() throws InterruptedException{
        String name = "out" + count.getAndAdd(1);
        log.info("{}-out写入缓存: {}", Thread.currentThread(), name);
        jdbcTemplate.execute("insert into bank (name, money) values ('"+ name + "', "
                + count.get() + ");");
        ...
        log.info("外部执行完毕!");
        Thread.sleep(2000);
        Assert.isTrue(Math.random() > 0.5, "外部事物回滚，子线程的事务不会回滚");
    }
    ...
    public void transExecute(boolean hasException) {
        String name = "in" + count.getAndAdd(1);
        log.info("{}-in写入缓存: {}", Thread.currentThread(), name);
        cache.set(name);

        try {
            jdbcTemplate.execute("insert into bank (`name`, `money`) values ('" + name + "', "
                    + count.get() + ");");
            // 处于事务中等待事务执行完，否则立即执行 run
