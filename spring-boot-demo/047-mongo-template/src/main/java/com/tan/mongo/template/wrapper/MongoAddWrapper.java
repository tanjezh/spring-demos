package com.tan.mongo.template.wrapper;
...
public class MongoAddWrapper {
    ...
    public void insert() {
        ...
        object.put("name", "tanblog");
        object.put("desc", "欢迎关注tanBlog");
        object.put("age", 28);

        // 插入一条document
        mongoTemplate.insert(object, COLLECTION_NAME);

        JSONObject ans = mongoTemplate.findOne(
                new Query(Criteria.where("name").is("tanblog")
                        .and("age").is(28)), JSONObject.class, COLLECTION_NAME);
    ...
    public void insertMany() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            record.put("wechart", "tanblog");
            record.put("blog", Arrays.asList("http://1.top", "http://2.top"));
            record.put("nums", 210);
            record.put("t_id", i);
            records.add(record);
        }
        ...
        // 查询插入的内容
        List<Map> result = mongoTemplate.find(
                new Query(Criteria.where("wechart").is("tanblog")),
    ...
    public void upsertNoMatch() {
        ...
        UpdateResult upResult = mongoTemplate.upsert(
                new Query(Criteria.where("name").is("tanblog").and("age").is(100)),
                new Update().set("age", 120).addToSet("add", "额外增加"), COLLECTION_NAME);
        System.out.println("nomatch upsert return: " + upResult);

        List<JSONObject> re = mongoTemplate.find(
                        new Query(Criteria.where("name").is("tanblog")
                                .and("age").is(120)), JSONObject.class, COLLECTION_NAME);
    ...
    public void upsertOneMatch() {
        ...
        UpdateResult result = mongoTemplate.upsert(
                new Query(Criteria.where("name").is("tanblog").and("age").is(120)),
                new Update().set("age", 100), COLLECTION_NAME);
        System.out.println("one match upsert return: " + result);

        List<JSONObject> ans = mongoTemplate.find(
                        new Query(Criteria.where("name").is("tanblog")
                                .and("age").is(100)), JSONObject.class, COLLECTION_NAME);
    ...
    public void upsertTwoMatch() {
        ...
        System.out.println("------------------------------------------");
        List<JSONObject> re = mongoTemplate.find(
                new Query(Criteria.where("name").is("tanblog").and("age").in(Arrays.asList(28, 100))),
        ...
        re = mongoTemplate.find(new Query(Criteria.where("name").is("tanblog")
                .and("age").is(120)), JSONObject.class, COLLECTION_NAME);
