...
public class MongoReadWrapper {
    ...
    public void specialFieldQuery() {
        Query query = new Query(Criteria.where("user").is("tanblog"));
        // 查询一条满足条件的数据
    ...
    public void andQuery() {
        Query query = new Query(Criteria.where("user").is("tanblog").and("age").is(18));
        Map result = mongoTemplate.findOne(query, Map.class, COLLECTION_NAME);
     ...
     */
    public void orQuery() {
        // 等同于 db.getCollection('demo').find({"user": "tanblog", $or: [{ "age": 18}, { "sign": {$exists: true}}]})
        Query query = new Query(Criteria.where("user").is("tanblog")
     ...
     */
    public void inQuery() {
        // 相当于: query xxx where id in (x,x,x)
    ...
    public void regexQuery() {
        Query query = new Query(Criteria.where("user").regex("^tanblog"));
        List<Map> result = mongoTemplate.find(query, Map.class, COLLECTION_NAME);
    ...
    public void sortQuery() {
        // sort查询条件，需要用with来衔接
        Query query = Query.query(Criteria.where("user").is("tanblog")).with(Sort.by("age"));
        List<Map> result = mongoTemplate.find(query, Map.class, COLLECTION_NAME);
    ...
    public void pageQuery() {
        // limit限定查询2条
        Query query = Query.query(Criteria.where("user").is("tanblog")).with(Sort.by("age")).limit(2);
        List<Map> result = mongoTemplate.find(query, Map.class, COLLECTION_NAME);
        System.out.println("query: " + query + " | limitPageQuery " + result);


        // skip()方法来跳过指定数量的数据
        query = Query.query(Criteria.where("user").is("tanblog")).with(Sort.by("age")).skip(2);
        result = mongoTemplate.find(query, Map.class, COLLECTION_NAME);
