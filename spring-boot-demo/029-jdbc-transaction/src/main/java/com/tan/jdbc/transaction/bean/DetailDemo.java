...
public class DetailDemo {
    ...
    @PostConstruct
    public void init(){
        String sql = "replace into money(id, name, money) values (320, '初始化', 200),"
                + "(330, '初始化', 300)," + "(340, '初始化', 400)," + "(350, '初始化', 500)";
    ...
    public boolean rrTransaction(int id){
        if(this.updateName(id)){
            this.query("rr: after updateName", id);
    ...
    public boolean ruTransaction(int id) throws InterruptedException{
        if(this.updateName(id)){
            this.query("ru: after updateName",id);
            Thread.sleep(2000);
            if(this.updateMoney(id)){
                return true;
            }
    ...
    public boolean rcTranaction(int id) throws InterruptedException {
        if (this.updateName(id)) {
            this.query("rc: after updateName", id);
            Thread.sleep(2000);
            if (this.updateMoney(id)) {
                return true;
            }
    ...
    public boolean serializeTransaction(int id) {
        if (this.updateName(id)) {
            this.query("serialize: after updateName", id);
            if (this.updateMoney(id)) {
                return true;
            }
        }
